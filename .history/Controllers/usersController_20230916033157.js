...
usersController.post("/login", async(req, res) => {
    ...
    // Validate the username and password using ternary operators.
    const isUsernameValid = regex.test(username) ? true : false;
    const isPasswordValid = regex.test(password) ? true : false;
    ...
    // Compare the password using ternary operator.
